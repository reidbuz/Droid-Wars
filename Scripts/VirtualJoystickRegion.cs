...
public class VirtualJoystickRegion : MonoBehaviour
{
	...
	private void ResetJoystick ()
	{
		...
		origin = new Vector2 ((Screen.width / 3) / 6.2f, (Screen.height / 3) / 3.7f); // added
		position = origin;
		gotPosition = false;
	}
	...
	{
		if (origin.x < (background.pixelInset.width / 6.2f) + 25) { //was divided by 2
			origin.x = (background.pixelInset.width / 6.2f) + 25;
		}
		if (origin.y < (background.pixelInset.height / 3.7f) + 25) { // was divided by 3
			origin.y = (background.pixelInset.height / 3.7f) + 25;
		}
	...
	private void Awake ()
	{
		gameObject.transform.localScale = new Vector3 (0.25f, 0.4f, 0);
		...
		backOBJ.transform.localScale = new Vector3 (-0.001f, -0.001f, -0.001f);
		...
		tapTimer = 0;
		length = 50;
		position = new Vector2 ((Screen.width / 3) / 6.2f, (Screen.height / 3) / 3.7f); 
		origin = position;
		gotPosition = false;
		EnableJoystick ();
		enable = true;
